...
int main(void)
{
  ...
  *******************************************************************************************************/
  ...
  while (1)
  {
	  //Seven_Segment(time_format(RTC->TR));
	  ...
	  //Clock Set Alarm Mode
		  ...
		  RCC->CR &= ~(0b1 << 8);//Allows write access
		  RCC->BDCR &= ~(0b1 << 15);//Disables clock
		  Seven_Segment(time_format(set_alarm));
		  GPIOD->ODR = 0x1 << display_index; //Tells what number is being edited
		  create_new_alarm();
	  ...
	  //Clock Display Time & Date Mode
		  ...
		  if(toggleDateTime == 0){
	 	  ...
	 	  //Displaying Date
	  	  ...
	  	  } else if(toggleDateTime == 2) {
	  		  Seven_Segment(time_format(RTC->ALRMAR));
	  		  GPIOD->ODR |= 0xFFFE;//Turns on all the red LEDS to show its displaying the alarm
	  		  Animate_On = 0;
	  	  }
		  //In this mode PC11 toggles the date and time
		  if(!(GPIOC->IDR & (1 << 11))){
			  (toggleDateTime == 2) ? (toggleDateTime = 0) : (toggleDateTime++); // toggles the bit to display date or time
		  	  HAL_Delay(15);
		  	  GPIOD->ODR = 0;
		  	  while(!(GPIOC->IDR & (1 << 11)));//Wait for button release
		  }
	  }
	  //PC10 Button toggles the clock mode
	  if(!(GPIOC->IDR & (1 <<10 ))){
		  if(toggleMode == 1){
		  ...
		  if(toggleMode == 3){
		  	RTC_Set_Alarm();
		  	 ...
		  	 set_alarm = RTC->ALRMAR;//log alarm
		  }
		  (toggleMode == 3) ? (toggleMode = 0) : (toggleMode++);//Increments the mode
	 ...
	 if((GPIOC->IDR & (0b1 << 15)) == 0b1 << 15){
		 ...
		 RTC->CR &= ~RTC_CR_ALRAIE; //disable interrupt
		 GPIOD->ODR &= ~0b10;
	 }
...
uint32_t time_format(uint32_t x)
{
	uint32_t time = x & 0x7F; //Seconds
	time |= 0xA << 8;
	time |= (x & 0x7F << 8) << 4; //Minutes
	time |= 0xA << 20;
	time |= (x & 0x3F << 16) << 8; //Hours
	return time;
}
...
void date_message_gen(uint32_t x){
	...
	Message[13] = (x & 0xF); //Day one's position
...
/*Writing new alarm handler*/
void create_new_alarm(){
	//Filtering invalid values for time
		if(!((GPIOC->IDR & 0xF) > 9)){
		  	  set_alarm &= ~(0xF << (display_index * 4));
			  set_alarm |= (GPIOC->IDR & 0xF) << (display_index * 4);//Reads the first four switches to set the time
		}
}
...
void RTC_Set_Alarm(void)
{
	...
	RTC->ALRMAR = set_alarm | RTC_ALRMAR_MSK4;

	RTC->CR |= RTC_CR_ALRAIE; // enable alarm interrupt (ALRAIE)
